...
% 创建结果保存目录
mkdir(fullfile("result\DBP\", mat2str(numberofSort)));

% 保存选择方式
save(fullfile("result\DBP\", mat2str(numberofSort), "choicesORTList"), "choiceSortList");
...
for i = 1: size(choiceSortList, 1)
    ...
    % 创建当前组合的保存目录
    mkdir(fullfile("result\DBP\", mat2str(numberofSort), mat2str(i)));
    ...
    % DBP归一化
    DBP_ref_list = [fixed_DBP_min; fixed_DBP_max];
    [~, DBPmapminmax] = mapminmax(DBP_ref_list', 0, 1);
    TrainDBP = mapminmax('apply', TrainDBPFirst', DBPmapminmax)';
    TestDBP = mapminmax('apply', TestDBPFirst', DBPmapminmax)';
    
    % 保存归一化参数
    save(fullfile("result\DBP\", mat2str(numberofSort), mat2str(i), "归一化参数"), ...
        'PPGmapminmax', 'DBPmapminmax', 'fixed_DBP_min', 'fixed_DBP_max');
    
    %% SVM参数优化与模型训练
    fprintf("正在优化SVM参数...\n");
    % [mse1, bestc1, bestg1] = SVMcgForRegress(TrainDBP, TrainSort,...
    %     0, 5, 5, 8, 5, 1, 1, 0.05);
    % 
    % [mse3, bestc, bestg] = SVMcgForRegress(TrainDBP, TrainSort,...
    %     log2(bestc1)-0.5, log2(bestc1)+0.5, log2(bestg1)-0.5, log2(bestg1)+0.5,...
    %     5, 0.1, 0.1, 0.05);
    bestc=0.707106781186548;bestg=78.7932424540746;
    fprintf("最优SVM参数 - C: %.4f, gamma: %.4f\n", bestc, bestg);
    
    cmd = ['-c ', num2str(bestc), ' -g ', num2str(bestg) , ' -s 3 -p 0.05 -b 1'];
    tic;
    model = svmtrain(TrainDBP, TrainSort, cmd);
    toc;
    
    % 保存模型与最优参数
    varargout = {model, bestc, bestg};
    save(fullfile("result\DBP\", mat2str(numberofSort), mat2str(i), "DBP"), 'varargout');
    
    % 测试集预测与结果计算
    fprintf("测试集预测中...\n");
    [TestPredDBP, AccTestPredDBP, ~] = svmpredict(TestDBP, TestSort, model, '-b 1');
    TestPredDBP = mapminmax('reverse', TestPredDBP', DBPmapminmax)';
    TestTrueDBP = mapminmax('reverse', TestDBP', DBPmapminmax)';
    DBP_MAE_test = mae(TestTrueDBP, TestPredDBP);
    DBP_STD_test = std(TestPredDBP - TestTrueDBP);
    fprintf("测试集结果 - MAE: %.2f, STD: %.2f\n", DBP_MAE_test, DBP_STD_test);
    
    % 保存测试集结果
    save(fullfile("result\DBP\", mat2str(numberofSort), mat2str(i), "TestPredDBP"), ...
        "DBP_MAE_test", "DBP_STD_test", "TestDBP", "TestPredDBP", "AccTestPredDBP");

                %% 绘制真实值与预测值对比图（折线图）- 改回长方形
    figure('Visible', 'off', 'Position', [100, 100, 500, 300]); % 改回长方形 500x300
    plot(1:length(TestTrueDBP), TestTrueDBP, 'b-', 'LineWidth', 1, 'DisplayName', 'True DBP');
    hold on;
    plot(1:length(TestPredDBP), TestPredDBP, 'r-', 'LineWidth', 1, 'DisplayName', 'Predicted DBP');
    xlabel('Sample Index');
    ylabel('DBP (mmHg)');
    title(sprintf('True vs Predicted DBP (Combination %d)', i));
    legend('Location', 'best');
    grid on;
    % 300 DPI 高清保存保持不变
    comp_filename = fullfile("result\DBP\", mat2str(numberofSort), mat2str(i), "Comparison.png");
    print(gcf, comp_filename, '-dpng', '-r300');
    close(gcf);

            %% 绘制 Bland-Altman 图
    figure('Visible', 'off', 'Position', [100, 100, 500, 500]); % 改为正方形
    diff = TestPredDBP - TestTrueDBP;
    meanVal = (TestTrueDBP + TestPredDBP) / 2;
    meanDiff = mean(diff);
    stdDiff = std(diff);
    ...
    xlabel('Mean of True and Predicted DBP (mmHg)', 'FontSize', 11);
    ...
    % 300 DPI 高清保存
    ba_filename = fullfile("result\DBP\", mat2str(numberofSort), mat2str(i), "BlandAltman.png");
    print(gcf, ba_filename, '-dpng', '-r300');
    close(gcf);

        %% 绘制相关性分析图
    figure('Visible', 'off', 'Position', [100, 100, 500, 500]);
    minVal = min([TestTrueDBP; TestPredDBP]);
    maxVal = max([TestTrueDBP; TestPredDBP]);
    ...
    scatter(TestTrueDBP, TestPredDBP, 20, 'b', 'filled', 'MarkerFaceAlpha', 0.6);
    hold on;
    plot([rangeMin, rangeMax], [rangeMin, rangeMax], 'k--', 'LineWidth', 1.2);
    coeffs = polyfit(TestTrueDBP, TestPredDBP, 1);
    xFit = linspace(rangeMin, rangeMax, 100);
    yFit = polyval(coeffs, xFit);
    plot(xFit, yFit, 'r-', 'LineWidth', 1.5);

    xlabel('True DBP (mmHg)', 'FontSize', 11);
    ylabel('Predicted DBP (mmHg)', 'FontSize', 11);
    ...
    R = corrcoef(TestTrueDBP, TestPredDBP);
    r = R(1,2);
    text(0.05, 0.9, sprintf('r = %.3f', r), 'Units', 'normalized', ...
        'FontSize', 12, 'BackgroundColor', 'white', 'EdgeColor', 'k');

    % 300 DPI 高清保存
    corr_filename = fullfile("result\DBP\", mat2str(numberofSort), mat2str(i), "Correlation.png");
